...
export default function Library() {
  ...
  async function openPiece(piece) {
    if (!piece.downloadUrl) {
      setErrorId(piece.id)
      setErrorMsg('No download URL — populate libraryIndex.js with real PDMX files.')
      return
    }
    ...
    <div style={{position:'absolute',inset:0,background:'var(--bg)',color:'var(--ink)',fontFamily:"'Sora',sans-serif",display:'flex',flexDirection:'column',overflow:'hidden'}}>
            ...
            return (
              <div
                ...
                style={{
                  ...
                  padding:'18px 22px',
                  background: isSelected ? 'rgba(0,255,200,0.06)' : 'transparent',
                  border: isSelected ? '1.5px solid var(--accent)' : '1px solid var(--border)',
                  borderRadius:12, cursor:'pointer', transition:'all 0.15s',
                  position:'relative', zIndex:1,
                }}
                onMouseEnter={e => { if (!isSelected) e.currentTarget.style.background = 'rgba(0,255,200,0.04)' }}
              ...
              >
                ...
                <div style={{display:'flex',flexDirection:'column',alignItems:'flex-end',gap:8,flexShrink:0}}>
                  ...
                  <button
                    onClick={e => { e.stopPropagation(); openPiece(piece) }}
                    disabled={isLoading}
                    style={{
                      padding:'8px 20px',
                      background: isLoading ? 'transparent' : 'var(--accent)',
                      border: isLoading ? '1.5px solid var(--accent)' : 'none',
                      color: isLoading ? 'var(--accent)' : '#000',
                      borderRadius:8, fontWeight:700, fontSize:12,
                      cursor: isLoading ? 'default' : 'pointer',
                      fontFamily:"'Sora',sans-serif",
                      boxShadow: isLoading ? 'none' : '0 4px 14px rgba(0,255,200,.2)',
      ...
      <div style={{flexShrink:0,padding:'10px 40px',borderTop:'1px solid var(--border)',fontSize:11,color:'var(--sub)',display:'flex',alignItems:'center',justifyContent:'space-between',gap:16}}>
        ...
        <span style={{flexShrink:0}}>
          <a href="https://github.com/KeerCode/Pianly-1.0.0/issues" target="_blank" rel="noreferrer" style={{color:'var(--accent)',textDecoration:'none'}}>Report an issue</a>
        </span>
      </div>
    </div>
  )
