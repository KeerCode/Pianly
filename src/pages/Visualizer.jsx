...
import { parseNoteTimeline, tickToSec as tempoMapTickToSec } from '../parsers/parseTimeline'
...
export default function Visualizer() {
  ...
  useEffect(() => {
    ...
    if (musicXml instanceof ArrayBuffer) {
      ...
      // MXL (ZIP) — unzip first, then parse inner XML
      const isMxl = header[0] === 0x50 && header[1] === 0x4b
      try {
        let xmlStr
        if (isMxl) {
          const files = unzipSync(new Uint8Array(musicXml))
          ...
          xmlStr = m ? new TextDecoder().decode(files[m[1]]) : null
        } else {
    ...
    async function setup() {
      ...
      try {
        unlistenOff = await listen('midi-note-off', (event) => {
          const noteName = event.payload?.note ?? event.payload
          const midi = noteNameToMidi(String(noteName))
          if (midi != null) heldMidiRef.current.delete(midi)
        })
      } catch (err) {
      ...
      try {
        await invoke('connect_midi', { deviceId })
      } catch (err) {
